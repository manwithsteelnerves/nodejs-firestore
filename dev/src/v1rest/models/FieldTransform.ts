...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    constructor(json: any) {
        ...
        this.appendMissingElements = new ArrayValue(json.appendMissingElements);
        }
        if (json.fieldPath) {
        this.fieldPath = json.fieldPath; //[Data format: ]
        }
        if (json.increment) {
        this.increment = new Value(json.increment);
        }
        if (json.maximum) {
        this.maximum = new Value(json.maximum);
        }
        if (json.minimum) {
        this.minimum = new Value(json.minimum);
        }
        if (json.removeAllFromArray) {
        this.removeAllFromArray = new ArrayValue(json.removeAllFromArray);
        }
...
export namespace FieldTransform {
    ...
    export const SetToServerValueEnum = {
        SERVERVALUEUNSPECIFIED: 'SERVER_VALUE_UNSPECIFIED' as SetToServerValueEnum,
        REQUESTTIME: 'REQUEST_TIME' as SetToServerValueEnum
    }
}
